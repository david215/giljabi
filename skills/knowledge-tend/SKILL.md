...
approval.** Present every proposal with its evidence (`file:line`, a diff, a count you read rather
than grepped) and apply the ones the user accepts; a layer that reshapes itself unsupervised is a
feedback loop with no test.
...
(no approval needed — it is generated by contract); a dangling relation names a page that was
